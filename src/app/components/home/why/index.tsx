...
function BoolCell({ v }: { v: boolean }) {
  return (
    <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full border ${v ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
      {v ? '✓' : '✕'}
    </span>
  )
}
...
export default function Why() {
  ...
  const cols: string[] = Array.isArray(compare?.columns) ? compare.columns : ['Funcionalidade', 'Planilha', 'Frota360']
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i} className='border-t border-white/10'>
                    ...
                    <td className='px-4 py-3 text-center'><BoolCell v={!!r.spreadsheet} /></td>
                    <td className='px-4 py-3 text-center'><BoolCell v={!!r.frota360} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
